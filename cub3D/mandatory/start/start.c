...
/*   Updated: 2024/05/08 17:48:23 by bmota-si         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*Claro, a função get_hit é responsável por determinar o ponto de interseção do raio com a parede mais próxima. Vamos examinar o que cada parte da função faz:
    ...
    while (!d->hit): Este loop executa até que uma parede seja atingida (d->hit seja verdadeiro). Enquanto isso, ele continua atualizando a posição do raio e verificando se ele atingiu uma parede.

    if (d->side_dist_x < d->side_dist_y): Verifica se a distância até o próximo lado x do bloco é menor do que a distância até o próximo lado y. Isso determina se o próximo passo do raio será ao longo do eixo x*/
...
/*    if (d->raydir_x < 0): Verifica se o componente x da direção do raio é negativo. Isso indica que o raio está indo para a esquerda no plano da tela.
        ...
        d->side_dist_x = (d->p_x - d->mapx) * d->delta_dist_x;: Calcula a distância do raio até o próximo lado x do bloco à esquerda. Isso é feito multiplicando a diferença entre a posição x atual da câmera (d->p_x) e a coordenada x do bloco atual (d->mapx) pela distância do raio entre os lados x dos blocos (d->delta_dist_x).
    ...
    O procedimento para determinar o lado do bloco ao longo do eixo y (d->raydir_y) é semelhante ao descrito acima, mas aplicado ao eixo y e usando d->stepy e d->side_dist_y para armazenar as informações correspondentes.
...
/*Esta função ray_calc é responsável por calcular os valores necessários para lançar um raio na cena. Aqui está uma explicação detalhada do que cada linha faz:
    ...
    d->camera_x = 2 * d->i / (double)WIDTH - 1;: Calcula a coordenada x da câmera no espaço de projeção. Isso é feito para normalizar as coordenadas da câmera entre -1 e 1, onde -1 representa a extremidade esquerda da tela e 1 representa a extremidade direita.

    d->raydir_x = d->dir_x + d->plane_x * d->camera_x;: Calcula o componente x da direção do raio lançado. Isso envolve a direção da câmera (d->dir_x) e o plano da câmera (d->plane_x), ajustados pela coordenada x da câmera no espaço de projeção.
    ...
    d->mapx = (int)d->p_x;: Calcula a coordenada x do bloco no qual o raio atinge primeiro. Isso é feito convertendo a coordenada x atual da câmera (d->p_x) para um número inteiro.
    ...
    if (d->raydir_x == 0) d->delta_dist_x = 1e30;: Calcula a distância do raio entre cada lado x dos blocos. Se d->raydir_x for zero, define d->delta_dist_x para um valor grande para evitar divisão por zero.

    else d->delta_dist_x = fabs(1 / d->raydir_x);: Calcula d->delta_dist_x como o inverso do componente x da direção do raio. Isso é feito para calcular a distância do raio entre os blocos ao longo do eixo x.

    if (d->raydir_y == 0) d->delta_dist_y = 1e30;: Calcula a distância do raio entre cada lado y dos blocos. Se d->raydir_y for zero, define d->delta_dist_y para um valor grande para evitar divisão por zero.

    else d->delta_dist_y = fabs(1 / d->raydir_y);: Calcula d->delta_dist_y como o inverso do componente y da direção do raio. Isso é feito para calcular a distância do raio entre os blocos ao longo do eixo y.
