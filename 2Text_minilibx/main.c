...
void    game(t_dir *dir)
{
    if(dir->key_press_up && dir->height > 0)
    {
        ...
        draw_square(dir->mlx_ptr, dir->win_ptr, dir->width, dir->height -= 100, 100, 0xFFFFFF);
        dir->key_press_up = 0;
    }
    ...
    {
        mlx_clear_window(dir->mlx_ptr, dir->win_ptr);
        draw_square(dir->mlx_ptr, dir->win_ptr, dir->width -= 100, dir->height, 100, 0xFFFFFF);
        dir->key_press_left = 0;
    }
...
void    draw_map(void *mlx_ptr, void *win_ptr, char *file_name, int size, int color)
{
    int fd;
    char *line;
    int width;
    ...
    while(line)
    {
        ...
        {
            if (line[i] == '1')
                draw_square(mlx_ptr, win_ptr, x += 100, j, size, color);
            else if (line[i] == 'P')
                draw_square(mlx_ptr, win_ptr, x += 100, j, size, 0xCED5F1);
            else
                x += 100;
            i++;
        }
...
int main(int ac, char **av)
{
    ...
    draw_map(dir->mlx_ptr, dir->win_ptr, av[1], 100, 0x3B58C8);
    draw_square(dir->mlx_ptr, dir->win_ptr, dir->width, dir->height, 100, 0xFFFFFF);
